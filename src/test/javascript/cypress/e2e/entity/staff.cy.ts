...
describe('Staff e2e test', () => {
  ...
  const staffSample = { name: 'Human middleware CSS', address: 'Circles Plastic', contactNo: 'Hawaii parsing Steel' };
  ...
  describe('new Staff page', () => {
    ...
    it('should create an instance of Staff', () => {
      cy.get(`[data-cy="regNo"]`).type('frame inno').should('have.value', 'frame inno');

      cy.get(`[data-cy="license"]`).select('Yes');

      cy.get(`[data-cy="gender"]`).select('Male');

      cy.get(`[data-cy="name"]`).type('Taiwan Division SMS').should('have.value', 'Taiwan Division SMS');

      cy.get(`[data-cy="address"]`).type('Granite Coordinator copying').should('have.value', 'Granite Coordinator copying');

      cy.get(`[data-cy="contactNo"]`).type('Identity neural').should('have.value', 'Identity neural');
